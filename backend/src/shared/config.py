...
class Settings(BaseModel):
    ...
    supabase_url: str = Field(
        default_factory=lambda: os.getenv("SUPABASE_URL") or "https://placeholder-url.supabase.co"
    )
    supabase_service_key: str = Field(
        default_factory=lambda: os.getenv("SUPABASE_SERVICE_KEY") or "placeholder-key"
    )
    supabase_anon_key: str = Field(
        default_factory=lambda: os.getenv("SUPABASE_ANON_KEY") or os.getenv("SUPABASE_SERVICE_KEY") or "placeholder-key"
    )
    ...
    stripe_webhook_secret: str = Field(
        default_factory=lambda: os.getenv("STRIPE_WEBHOOK_SECRET") or "whsec_placeholder"
    )
    
    app_env: str = Field(
        default_factory=lambda: os.getenv("APP_ENV") or "production"
    )
