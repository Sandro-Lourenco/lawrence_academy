...
class GetLessonStreamUseCase:
    ...
    async def execute(
        self, user_id: str, role: str, course_id: str, lesson_id: str
    ) -> str:
        # 1. Recuperar o caminho de armazenamento do HLS da aula
        storage_path = await self.repository.get_lesson_stream_path(
            course_id, lesson_id
        )
        if not storage_path:
        ...
        if not has_access:
            raise AuthorizationError(
                "Acesso negado. Assinatura inativa para este curso."
            )
