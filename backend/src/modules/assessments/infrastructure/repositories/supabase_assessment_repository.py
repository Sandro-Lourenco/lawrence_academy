...
class SupabaseAssessmentRepository(AssessmentRepository):
    ...
    def _map_row(self, row: dict) -> TaskSubmission:
        return TaskSubmission(
            ...
            graded_at=datetime.fromisoformat(row["graded_at"].replace("Z", "+00:00")) if row.get("graded_at") else None
        )
    ...
    async def review(self, submission_id: str, score: float, teacher_feedback: str, graded_by: str) -> TaskSubmission:
        update_data = {
            "score": score,
        ...
        res = self.client.table("task_submissions").update(update_data).eq("id", submission_id).execute()
        if not res.data:
