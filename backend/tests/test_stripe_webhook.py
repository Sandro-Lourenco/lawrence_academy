...
def test_stripe_webhook_idempotency_duplicate(mock_supabase, mock_construct):
    ...
    mock_supabase.table().insert().execute.side_effect = Exception("duplicate key value violates unique constraint")
...
@patch("src.modules.payments.application.process_webhook.stripe.Customer.create_balance_transaction")
def test_stripe_webhook_payment_succeeded_referral(mock_create_balance, mock_retrieve_sub, mock_supabase, mock_construct):
    ...
    mock_supabase.table.side_effect = lambda table_name: mock_table_routing(table_name, mock_lock_res, mock_profile_res, mock_ind_sub_res)
