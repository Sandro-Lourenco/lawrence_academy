...
class CourseWizardController
  ...
  Future<bool> uploadCourseMedia({
    required String assetType, required String filePath, required String filename,
    required int sizeBytes, required String contentType, String altText = '',
  }) async {
    if (_courseId == null || !state.hasValue) return false;
    state = AsyncValue.data(state.value!.copyWith(
      isUploading: true, uploadMessage: assetType == 'cover' ? 'Enviando capa...' : 'Enviando trailer...', error: null,
    ));
    try {
      await ref.read(teacherCourseUseCasesProvider).uploadCourseMedia(
        courseId: _courseId!, assetType: assetType, filePath: filePath,
        filename: filename, sizeBytes: sizeBytes, contentType: contentType,
        altText: altText,
      );
      final refreshed = await ref.read(teacherCourseUseCasesProvider).getCourse(_courseId!);
      state = AsyncValue.data(state.value!.copyWith(course: refreshed, isUploading: false,
        uploadMessage: assetType == 'cover' ? 'Capa enviada.' : 'Trailer em processamento.'));
      return true;
    } catch (e) {
      state = AsyncValue.data(state.value!.copyWith(isUploading: false, error: e.toString(), uploadMessage: null));
      return false;
    }
  }
  ...
  Future<bool> saveLessonBlock(String lessonId, Map<String, dynamic> data, {String? blockId}) async {
    if (_courseId == null || !state.hasValue) return false;
    state = AsyncValue.data(state.value!.copyWith(isSaving: true, uploadMessage: 'Salvando bloco...', error: null));
    try {
      final usecases = ref.read(teacherCourseUseCasesProvider);
      if (blockId == null) {
        await usecases.createLessonBlock(_courseId!, lessonId, data);
      } else {
        await usecases.updateLessonBlock(_courseId!, lessonId, blockId, data);
      }
      final refreshed = await usecases.getCourse(_courseId!);
      state = AsyncValue.data(state.value!.copyWith(course: refreshed, isSaving: false, uploadMessage: 'Bloco salvo.'));
      return true;
    } catch (error) {
      state = AsyncValue.data(state.value!.copyWith(isSaving: false, error: error.toString(), uploadMessage: null));
      return false;
    }
  }
  Future<List<LessonBlock>> listLessonBlocks(String lessonId) async {
    if (_courseId == null) return const [];
    return ref.read(teacherCourseUseCasesProvider).listLessonBlocks(_courseId!, lessonId);
  }
  ...
  Future<bool> duplicateLessonBlock(String lessonId, String blockId) async {
    ...
    try {
      final usecases = ref.read(teacherCourseUseCasesProvider);
      await usecases.duplicateLessonBlock(_courseId!, lessonId, blockId);
      final refreshed = await usecases.getCourse(_courseId!);
      state = AsyncValue.data(state.value!.copyWith(course: refreshed));
      return true;
    } catch (error) { state = AsyncValue.data(state.value!.copyWith(error: error.toString())); return false; }
  }
